...
public class QuickSearch extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String live_only = request.getParameter("live_only");
        
        boolean search_all = true;
        if ((live_only != null) && (live_only.equals("yes")))
        { search_all = false; }
        ...
        ArrayList<Integer> title_search_results = new ArrayList<Integer>();
        ArrayList<Integer> subtitle_search_results = new ArrayList<Integer>();
        ArrayList<Integer> desc_search_results = new ArrayList<Integer>();
        if (rover_only != null)
        {
            String title_search_constraints = "lower(title) LIKE ? AND is_rover = ?";
            String subtitle_search_constraints = "lower(subtitle) LIKE ? AND is_rover = ?";
            String desc_search_constraints = "lower(quick_description) LIKE ? AND is_rover = ?";
            ...
            subtitle_search_results = Query.find("moe_resource", subtitle_search_constraints, search_args);
            desc_search_results = Query.find("moe_resource", desc_search_constraints, search_args);
        }
        else
        { 
            title_search_results = Query.findWithOperator("moe_resource", "title", title_to_search, Enumerators.BBComparisonOperator.CONTAINS); 
            subtitle_search_results = Query.findWithOperator("moe_resource", "subtitle", title_to_search, Enumerators.BBComparisonOperator.CONTAINS); 
        ...
        try {
            ...
            while (it.hasNext())
            {
                ...
                if ((is_live == 1) || (search_all))
                {
                    ...
                    String this_title = this_res.getTitleAndSubtitle();
