 ...
 */
  public EasyContext(HttpServletRequest request) {
 ...
 */
  ...
  public BbResourceBundle getResourceBundle() {
    ...
    if (this.resourceBundle == null) {
      //this.resourceBundle = BundleManagerFactory.getInstance().
    }
    ...
    if (getNodeSupport() && (this.context != null)) {
      if (this.context.hasCourseContext()) {
        try {
          ...
          Logger.getLogger(EasyContext.class.getName()).log(Level.SEVERE, null, e);
        }
    ...
    if (configFile != null) {
      if (!configFile.exists()) {
        if (!configFile.mkdirs()) {
          Logger.getLogger(EasyContext.class.getName()).log(Level.SEVERE, "Unable to create directories");
          configFile = null;
        }
      }
    }
...
// Function to initialises the logger and sets the default logging level to errors only.
  private static void getLogger() {
    ...
    if (logger == null) {
      ...
      setLogDebug(false);
      if (logPrefix == null) {
        EasyContext b2Context = new EasyContext(null);
